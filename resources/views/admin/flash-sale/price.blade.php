            ...
            <div class="panel">
                <div class="panel-content">
                    <div class="table-responsive">
                        ...
                        <form action="{{ route('flash-sale-price') }}" method="post">
                            @csrf
                            <div class="form-group col-md-8">
                                <label for="inputAddress">Campaign</label>
                                <input type="text" class="form-control" name="cam_name" value="{{ $allPublishedProducts->first()->cam_name }}"  placeholder="Campaign Name">
                            </div>
                            <div class="form-row">
                                <div class="form-group col-md-2">
                                    <label for="">Date</label>
                                    <input type="date" class="form-control" name="cam_date" value="{{ $allPublishedProducts->first()->cam_date }}">
                                </div>
                                <div class="form-group col-md-2">
                                    <label for="inputPassword4">Time</label>
                                    <input type="time" class="form-control" name="cam_time" step="2" value="{{ $allPublishedProducts->first()->cam_time }}" style="margin-bottom: 30px">
                                </div>
                                <hr>
                            </div>
...
{{--                                        </td>--}}
                                        <td><input type="text" name="flash_sale_price[]" value="{{ $allPublishedProduct->flash_sale_price }}"></td>
                                        <input type="hidden" name="flash_id[]" value="{{ $allPublishedProduct->flash_id }}">
                            ...
                            @foreach($allPublishedProductsID as $item)
                            <input type="hidden" name="flash_sale_item[]" value="{{ $item->id }}">
