                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($coupon->publication_status==1)
                                            <a href="{{route('coupon-display-status',['id'=>$coupon->id])}}" class="btn btn-success btn-xs" title="Published">
                                                <span class="glyphicon glyphicon-arrow-up"></span>
                                            </a>
                                        @else
                                            <a href="{{route('coupon-display-status',['id'=>$coupon->id])}}" class="btn btn-warning btn-xs" title="UnPublished">
                                                <span class="glyphicon glyphicon-arrow-down"></span>
                                            </a>
                                        @endif
                                        <a href="{{route('edit-coupon-display',['id'=>$coupon->id])}}" class="btn btn-primary btn-xs">
                                            <span class="glyphicon glyphicon-edit"></span>
                                        </a>
                                        <a href="{{route('delete-coupon-display',['id'=>$coupon->id])}}" class="btn btn-danger btn-xs" onclick="return confirm('Are you sure to Delete this!!')" title="delete">
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('coupon-display') }}" method="post" enctype="multipart/form-data">
                        @csrf
